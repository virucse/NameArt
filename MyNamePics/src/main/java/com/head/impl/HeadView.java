...
public class HeadView extends FrameLayout {
    ...
    private final List<BitmapHeadIcon> icons = new ArrayList<>(4);
    ...
    private BitmapHeadIcon currentIcon;
    ...
    @HeadView.ActionMode
    private int currentMode = HeadView.ActionMode.NONE;
    ...
    public void configDefaultIcons() {
        BitmapHeadIcon deleteIcon = new BitmapHeadIcon(
                ContextCompat.getDrawable(getContext(), R.drawable.sticker_ic_close_white_18dp),
                BitmapHeadIcon.LEFT_TOP);
        deleteIcon.setIconEvent(new DeleteHeadIconEvent());
        BitmapHeadIcon zoomIcon = new BitmapHeadIcon(
                ContextCompat.getDrawable(getContext(), R.drawable.sticker_ic_scale_white_18dp),
                BitmapHeadIcon.RIGHT_BOTOM);
        zoomIcon.setIconEvent(new ZoomHeadIconEvent());
        /*BitmapHeadIcon flipIcon = new BitmapHeadIcon(
                ContextCompat.getDrawable(getContext(), R.drawable.sticker_ic_flip_white_18dp),
                BitmapHeadIcon.RIGHT_TOP);
        ...
        if (handlingSticker != null && !locked && (showBorder || showIcons)) {
            ...
            if (showIcons) {
                ...
                for (int i = 0; i < icons.size(); i++) {
                    BitmapHeadIcon icon = icons.get(i);
                    switch (icon.getPosition()) {
                        case BitmapHeadIcon.LEFT_TOP:

                            configIconMatrix(icon, x1, y1, rotation);
                            break;

                        case BitmapHeadIcon.RIGHT_TOP:
                            configIconMatrix(icon, x2, y2, rotation);
                            break;

                        case BitmapHeadIcon.LEFT_BOTTOM:
                            configIconMatrix(icon, x3, y3, rotation);
                            break;

                        case BitmapHeadIcon.RIGHT_BOTOM:
    ...
    protected void configIconMatrix(@NonNull BitmapHeadIcon icon, float x, float y,
                                    float rotation) {
        icon.setX(x);
        icon.setY(y);
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                ...
                if (handlingSticker != null && isInStickerArea(handlingSticker, event.getX(1),
                    ...
                    currentMode = HeadView.ActionMode.ZOOM_WITH_TWO_FINGER;
                }
                break;
            ...
            case MotionEvent.ACTION_POINTER_UP:
                if (currentMode == HeadView.ActionMode.ZOOM_WITH_TWO_FINGER && handlingSticker != null) {
                    if (onStickerOperationListener != null) {
                        onStickerOperationListener.onStickerZoomFinished(handlingSticker);
                    }
                }
                currentMode = HeadView.ActionMode.NONE;
                break;
        }
    ...
    protected boolean onTouchDown(@NonNull MotionEvent event) {
        currentMode = HeadView.ActionMode.DRAG;
        ...
        if (currentIcon != null) {
            currentMode = HeadView.ActionMode.ICON;
    ...
    protected void onTouchUp(@NonNull MotionEvent event) {
        ...
        if (currentMode == HeadView.ActionMode.ICON && currentIcon != null && handlingSticker != null) {
            currentIcon.onActionUp(this, event);
        }

        if (currentMode == HeadView.ActionMode.DRAG
                && Math.abs(event.getX() - downX) < touchSlop
                && Math.abs(event.getY() - downY) < touchSlop
                && handlingSticker != null) {
            currentMode = HeadView.ActionMode.CLICK;
            if (onStickerOperationListener != null) {
        ...
        if (currentMode == HeadView.ActionMode.DRAG && handlingSticker != null) {
            if (onStickerOperationListener != null) {
    ...
    protected void handleCurrentMode(@NonNull MotionEvent event) {
        switch (currentMode) {
            case HeadView.ActionMode.NONE:
            case HeadView.ActionMode.CLICK:
                break;
    ...
    @Nullable
    protected BitmapHeadIcon findCurrentIconTouched() {
        for (BitmapHeadIcon icon : icons) {
    ...
    @NonNull
    public HeadView addSticker(@NonNull Sticker sticker) {
        return addSticker(sticker, Sticker.Position.CENTER);
    }

    public HeadView addSticker(@NonNull final Sticker sticker,
   ...
   /* public void save(@NonNull File file) {
    ...
    @NonNull
    public HeadView setLocked(boolean locked) {
        this.locked = locked;
        invalidate();
        return this;
    }
    ...
    @NonNull
    public HeadView setMinClickDelayTime(int minClickDelayTime) {
    ...
    @NonNull
    public HeadView setConstrained(boolean constrained) {
    ...
    @NonNull
    public HeadView setOnStickerOperationListener(
    ...
    @NonNull
    public List<BitmapHeadIcon> getIcons() {
        return icons;
    }

    public void setIcons(@NonNull List<BitmapHeadIcon> icons) {
        this.icons.clear();
    ...
    public void setShowBorder(boolean showb) {
        showBorder = showb;
    }

    @IntDef({
            HeadView.ActionMode.NONE, HeadView.ActionMode.DRAG, HeadView.ActionMode.ZOOM_WITH_TWO_FINGER, HeadView.ActionMode.ICON,
            HeadView.ActionMode.CLICK
    })
