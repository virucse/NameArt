...
(*env)->
ReleaseIntArrayElements(env, arrIn, pixels,
0);
// return result;
}
...
// Get image info
if (
AndroidBitmap_getInfo(env, bitmapIn,
&infoIn) != ANDROID_BITMAP_RESULT_SUCCESS) {
...
// Lock all images
if (
AndroidBitmap_lockPixels(env, bitmapIn,
&pixels) != ANDROID_BITMAP_RESULT_SUCCESS) {
